...
/*void Internet::Post(const std::string& url, const std::string& jsonData) {
    HINTERNET hInternet = InternetOpenUrlA("PraxClient/1.0", INTERNET_OPEN_TYPE_DIRECT, nullptr, nullptr, 0);
    if (!hInternet) {
...
std::string Internet::GetSillyPicture(const std::string& endpoint, bool nsfw) {
    std::string category = nsfw ? ("nsfw/") : ("sfw/");
    std::string jsonString = Get(("https://api.waifu.pics/") + category + endpoint);
